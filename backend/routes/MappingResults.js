...
const MappingDetail = require('../models/MappingBoard');
...
router.get('/mapping-board-details/:id', async (req, res) => {
  console.log("Mapping Result from db")
  const boardId = req.params.id; // Assuming you're using a unique ID for the board

  try {
    const boardDetails = await MappingDetail.findById(boardId); // Fetch by unique ID
    if (!boardDetails) {
      return res.status(404).json({ message: 'Board not found' });
    }
    res.json(boardDetails); // Return the board details
