                        ...
                        <div key={jobType}>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                {Array.isArray(filteredJobs[jobType]) ? (
                                            ...
                                            <div className="flex justify-between items-center">
                                                <div className="space-y-2">
                                                    ...
                                                    <p className="text-sm">
                                                        ...
                                                        {new Date(job.publishDate).toLocaleDateString('en-GB', {day:'2-digit', month:'short', year: 'numeric',})}
                                                    </p>
                                                    <p className="text-sm">
                                                        <strong>Last Date:</strong>{' '}
                                                        {new Date(job.lastDate).toLocaleDateString('en-GB',{day:'2-digit',month:'short', year: 'numeric',})}
                                                    </p>
                                                    {/* <p className="text-sm">
                                                ...
                                                {new Date(job.lastDate) > new Date() ? (
                                                    <button
                                                        className="bg-green-500 text-white font-semibold text-lg px-4 py-2 rounded hover:bg-green-600 ml-auto"
                                                        onClick={() => handleCreateBoard(job)}
                                                        disabled={disabledJobIds.includes(job._id)}
                                                    >
                                                        Board
                                                    </button>
                                                ) : (
                                                    <button className="bg-blue-500 text-white font-semibold text-lg px-4 py-2 rounded hover:bg-green-600 ml-auto"
                                                    >Update</button>
                                                )}
                                            </div>
                                        </div>
                                    ...
                                    <p className="text-gray-500">No jobs found under this category.</p>
                                )}
                            </div>
                        </div>
                    ))
                ) : (
                    <p className="text-gray-500">No jobs found for the selected month and year.</p>
                )}
            </div>
        </div>
    );
